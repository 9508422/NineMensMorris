...
public class Piece {
     ...
     *
     * @param player the Player that the Piece is assigned too
     */
    ...
    /**
     * @return if the Piece has a legal move
     */
    public boolean hasLegalMove() {
        if (hasSpot()) {
            for (Spot spot : this.spot.getNeighbours()) {
                if (!spot.hasPiece()) {
                    return true;
                }
            }
    ...
    public boolean validRemove() {
        return hasSpot() && ((!player.allPiecesInMill() && !inMill) || player.allPiecesInMill());
    }
